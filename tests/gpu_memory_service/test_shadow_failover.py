...
def _wait_for_directory_writer(directory, manifest, expected, timeout=10.0):
    """Wait until deferred metadata publication establishes its writer."""
...
def _wait_for_hbm_inventory(directory, writer, epoch, scope, timeout=10.0):
    """Wait for deferred READY publications from the engine worker."""
    deadline = time.monotonic() + timeout
    while True:
    ...
    with GMSProcessManager(request, VLLMWithGMSProcess, kv_directory=True) as manager:
        ...
        with DaemonClient(manager.kv_directory_socket) as directory:
            ...
            protected, rejected = _wait_for_hbm_inventory(
                directory, writer, epoch, scope="vllm"
            )
            assert not rejected
