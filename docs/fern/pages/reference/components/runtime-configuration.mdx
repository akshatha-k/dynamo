...
<ParamField path="--engine-request-limit" type="integer" default="null">
  Maximum requests a process admits concurrently, across every endpoint it serves. Setting a positive
  integer pins that process's admission limit. When unset, the limit is derived from the first usable
  capacity reported by a non-LoRA base model card registered in the process,
  `ceil(3/2 x max_num_seqs x data_parallel_size)`, and falls back to exactly `10000` when no card
  reports a usable `max_num_seqs`. Any component that registers a base card can supply that value,
  routers included; the runtime does not restrict it to engines. When the slots and the Dynamo
  overflow queue are full, the process refuses the request before it reaches the engine and the
  request fails.

  Environment variable: `DYN_ENGINE_REQUEST_LIMIT`
</ParamField>

<ParamField path="DYN_DYNAMO_REQUEST_QUEUE_LIMIT" type="integer" default="40000">
  Advanced overflow-queue size for requests waiting in Dynamo before entering the engine. Applies
  whether or not the engine limit is set explicitly. The effective per-worker-process cap is the
  engine limit plus this queue limit, counted across the TCP and NATS request planes together and
  across every endpoint the process serves.
</ParamField>

<ParamField path="DYN_DYNAMO_REQUEST_QUEUE_TIMEOUT_MS" type="integer" default="5000">
  Advanced maximum time, in whole milliseconds, that a request may wait in the Dynamo overflow queue
  before the worker gives up on it. Environment-only: there is no command-line flag. A positive
  integer replaces the default, down to `1`; an unset, unparseable or non-positive value leaves the
  default in place.

  This bounds queue residence only. It does not limit how long an admitted request may run, and a
  request that takes a free engine slot immediately never carries a deadline. An expired request is
  removed from the queue at once and refused exactly as a full queue refuses one. It has an effect
  only while `DYN_DYNAMO_REQUEST_QUEUE_ENABLE_CONTROLLED_DELAY` is enabled.
</ParamField>

<ParamField path="DYN_DYNAMO_REQUEST_QUEUE_ENABLE_CONTROLLED_DELAY" type="boolean" default="true">
  Advanced switch for whether a queued request that outlives `DYN_DYNAMO_REQUEST_QUEUE_TIMEOUT_MS` is
  rejected. Enabled, a request that passes its deadline leaves the queue at once and is refused.
  Disabled, nothing leaves the queue for age: a request may wait longer than the delay and still be
  admitted in its FIFO turn, and the queue length limit becomes the only backpressure.

  Accepts `1`/`true`/`on`/`yes` and `0`/`false`/`off`/`no`, case-insensitively. Environment-only:
  there is no command-line flag. An unset, empty or unrecognized value leaves it enabled.
</ParamField>

<ParamField path="DYN_DYNAMO_REQUEST_QUEUE_ENABLE_ADAPTIVE_LIFO" type="boolean" default="true">
  Advanced switch for which queued request the worker admits after it has rejected one. Enabled, a
  freed slot that followed a rejection goes to the newest queued request — the one with the most of
  its queue delay left — and the admission after that goes to the oldest again. Disabled, a freed
  slot always goes to the oldest queued request.

  Accepts the same vocabulary and default as the switch above, and is likewise environment-only. This
  governs selection only: it does not enable or disable the queue delay, change which requests are
  rejected, or change what a rejected request is refused with. It has no effect unless Controlled
  Delay has actually rejected a queued request.
</ParamField>
