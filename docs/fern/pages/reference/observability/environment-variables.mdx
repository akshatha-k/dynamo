...
<ParamField path="DYN_BENCH_KV_WARMUP_STATE" type="string" default="auto">
  Recurrent-state hand-off on hybrid layouts. Attention layers read the warm-up chain's KV by position; a Mamba / linear-attention layer keeps one running-state block per request, which the measured request borrows from its chain. `copy` forks that block (copy-on-write, one extra block per request and state group; needs a vLLM whose KV-cache managers support CoW), `inplace` hands the chain's own block to the measured request, `auto` copies when the manager can and the pool holds the forks, otherwise runs in place. The chosen mode is recorded as `state_mode` in the KV warm-up metadata and per row as `kvwarm_state_copy` / `kvwarm_state_inplace`, alongside `kvwarm_state_depth=N`, the chain depth the borrowed state summarises. An explicit `copy` on a vLLM without CoW skips the warm-up with `state_copy_unsupported`.
</ParamField>
