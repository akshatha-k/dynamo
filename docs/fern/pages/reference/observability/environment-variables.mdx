...
<ParamField path="DYN_BENCH_KV_WARMUP" type="string" default="on">
  KV warm-up master switch. When on, decode benchmark points on expert-parallel MoE models read KV produced by real prefill chains instead of self-injected synthetic KV. Set `off` to force the legacy synthetic-KV path. Ineligible configurations (dense models, no expert parallelism, prefix caching disabled, or a seeding dataset that is unavailable, empty, too shallow for the model length, or without a loadable tokenizer) skip the warm-up automatically and record the reason. Under attention data parallelism the decision is group-wide: if any rank is ineligible, every rank uses synthetic KV; if any rank's warm-up stage fails to build or cannot reserve its shadow blocks, every rank falls back to synthetic KV for that stage. Hybrid layouts whose recurrent-state layers (Mamba / linear attention) use vLLM's `align` cache mode are supported; how a measured request obtains its state block is recorded as `state_mode` (see `DYN_BENCH_KV_WARMUP_STATE`). Other Mamba cache modes skip the warm-up with `hybrid_state_layers_unsupported`.
</ParamField>

<ParamField path="DYN_BENCH_KV_WARMUP_STATE" type="string" default="auto">
  Recurrent-state hand-off on hybrid layouts. Attention layers read the warm-up chain's KV by position; a Mamba / linear-attention layer keeps one running-state block per request, which the measured request borrows from its chain. `copy` forks that block (copy-on-write, one extra block per request and state group; needs a vLLM whose KV-cache managers support CoW), `inplace` hands the chain's own block to the measured request, `auto` copies when the managers can and otherwise runs in place. The mode is fixed by the layout and the vLLM build for the whole run, never by the free pool: a pool too small for it skips the point like any other shadow-block shortfall. It is recorded as `state_mode` in the KV warm-up metadata and per row as `kvwarm_state_copy` / `kvwarm_state_inplace`, alongside `kvwarm_state_depth=N`, the chain depth the borrowed state summarises. `inplace` lets the measured request write into the chain's state block: later points that borrow the same chain in that stage, and the next generation's deeper chain that resumes from it through the prefix cache, start from the advanced state (timing-neutral, but no longer the pure chain state). Which modes a vLLM supports depends on its KV-cache managers: without CoW only `inplace` is possible; a build that hashes partial tails at a finer granularity than the state block (the vLLM 0.28.0 line) would corrupt the chain's prefix entry from a shared state block, so only `copy` is used there. An explicit mode the build cannot serve skips the warm-up with `state_copy_unsupported` or `state_inplace_unsupported`; an unparseable value skips it with `state_mode_invalid` (on layouts without recurrent-state groups the variable is ignored).
</ParamField>
