...
<ParamField path="multinode" type="MultinodeSpec">
  Configures a `worker`, `prefill`, or `decode` component that spans multiple pods. Other component types reject this field. See [Multinode Orchestration](../../kubernetes/installation/multinode-orchestration.md).
</ParamField>
...
<ParamField path="roles" type="[]ComponentRoleSpec">
  Optional explicit structure of a compound component. In this release, roles are supported for multinode components and must contain exactly one `leader` and one `worker`. Their cardinality remains derived from `multinode.nodeCount`. Omit the field to retain the implicit leader/worker layout.
</ParamField>
