...
def default_kv_lease_namespace_suffix(engine: str) -> str:
    """Stable engine-local lease namespace used by allocators and failover."""
    normalized = engine.lower().replace("-", "_")
    return {"vllm": "block-pool", "sglang": "page-pool"}.get(normalized, "kv")
